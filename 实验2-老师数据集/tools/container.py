...
class Container:
    def __init__(self, l=587, w=233, h=220):
        self.l = l
        self.w = w
        self.h = h
    ...
    def putBox_Advance(self, box):
        # 动态算法找最小面作为底面
        # 来充分利用高度空间
        [boxL, boxW] = box.getMinSurface() # boxW是最小的
        boxH = box.getMaxH()
        # 用当前集装箱已用空间比值确定i的遍历起始点
        base_line = max(0, (self.getTotalCapacity()-self.getRemainingCapacity())/self.getTotalCapacity()-0.1)
        # print(base_line)
        begin = int(base_line*self.l)
        # print(begin)
        for j in range(0, self.w - boxW + 1, 1):
            for i in range(0, self.l - boxL + 1, 1):
                if np.all(self.surface[i: i+boxL, j: j+boxW]>=boxH):
                    curCapacity = np.min(self.surface[i: i+boxL, j: j+boxW])
                    self.updateSurface(i, boxL+i, j, j+boxW, curCapacity, boxH)
                    print('当前盒子放在', i, boxL+i, j, j+boxW)
